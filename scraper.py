# -*- coding: utf-8 -*-
"""
Détecteur de nouvelles offres de logement.
...
def fetch_listings(site: dict) -> dict:
    ...
    des annonces/résidences actuellement présentes sur la page du site.
...
def check_site(site: dict) -> None:
    ...
    if not previous:
        ...
        for listing_id in new_ids:
            print(f"    -> NOUVELLE ANNONCE : {current[listing_id]['title']}")
            send_notification(name, current[listing_id]["title"], listing_id)
        if not new_ids:
